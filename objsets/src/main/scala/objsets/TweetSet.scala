...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def bestTweet(tweet1: Tweet, tweet2: Tweet): Tweet = {
      if (tweet1.retweets > tweet2.retweets) tweet1
      else tweet2
    }

    if (left.isEmpty && right.isEmpty) {
      println(elem.user + "--> empty")
      elem
    }
    else if (left.isEmpty) bestTweet(elem, right.mostRetweeted)
    else {
      println(left.mostRetweeted.retweets + " --> other" + right.mostRetweeted.retweets)
      bestTweet(bestTweet(elem, left.mostRetweeted), right.mostRetweeted)
    }
  }

...
trait Solver extends GameDef {
  ...
  //b.legalNeighbors => List((Block(Pos(1,2),Pos(1,3)), Right), (Block(Pos(2,1),Pos(3,1)), Down))
  /* LazyList(
     (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
     (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
   )*/
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    (for {(blockNeighbor, moveNeighbor) <- b.legalNeighbors
          } yield (blockNeighbor, moveNeighbor :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    for ((block, movements) <- neighbors
         if !explored.contains(block)) yield (block, movements)
  }
  ...
  /*initial = LazyList(
    ...
    (Block(Pos(2,1),Pos(3,1)), List(Left,Down)
    explored = Set(Block(Pos(1,2),Pos(1,3)),Block(Pos(1,2),Pos(1,3)),Block(Pos(1,2),Pos(1,0
    return = LazyList(
    (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
    (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))*/
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val cabecereaBlock: Block = initial.head._1
      val cabecereaMove: List[Move] = initial.head._2

      val nuevosVecinos: LazyList[(Block, List[Move])] =
        newNeighborsOnly(neighborsWithHistory(cabecereaBlock, cabecereaMove), explored)

       initial.head #:: from(initial.tail ++ nuevosVecinos , explored ++ nuevosVecinos.map(_._1))
    }
  }
